...
SEARXNG_URL = os.getenv('SEARXNG_URL')  # Получаем URL из переменной окружения
API_KEY = None
...
class SearchClientBase:
    """Базовый класс для поисковых клиентов"""
    def __init__(self, cache: CompressedCache):
        self.cache = cache
        self.session = requests.Session()
        self.last_request_time = 0

    def _wait_for_next_request(self):
        """Ожидание 1 секунду между запросами"""
    ...
    def _generate_cache_key(self, query: str, prefix: str = "") -> str:
        return hashlib.md5(f"{prefix}_{query}".encode('utf-8')).hexdigest()
...
class BraveSearchClient(SearchClientBase):
    ...
    def search(self, query: str) -> List[Dict]:
        cache_key = self._generate_cache_key(query, "brave")
        cached_result = self.cache.get(cache_key)
        if cached_result:
            logger.info(f"Brave cache hit for query: {query}")
        ...
        for attempt in range(self.retry_count):
            try:
                ...
                if response.status_code == 429:  # Too Many Requests
                    logger.warning("Rate limit hit, retrying in 1 second...")
                    time.sleep(1)
                    continue
                ...
                logger.error(f"Brave API request error (attempt {attempt + 1}/{self.retry_count}): {e}")
...
class SearxNGClient(SearchClientBase):
    """Клиент для работы с локальным SearxNG"""
    def __init__(self, base_url: str, cache: CompressedCache):
        super().__init__(cache)
        self.base_url = base_url.rstrip('/')
        self.retry_count = 3

    def search(self, query: str) -> List[Dict]:
        cache_key = self._generate_cache_key(query, "searxng")
        cached_result = self.cache.get(cache_key)
        if cached_result:
            ...
            try:
                ...
                for result in data.get("results", []):
                    ...
                    if not any(site.lower() in url_lower for site in EXCLUDED_SITES):
                        results.append({
                            "title": result.get("title", ""),
                            "page_age": result.get("published_date", ""),
                            "url": url_lower
                        })
                ...
                logger.error(f"SearxNG search error (attempt {attempt + 1}/{self.retry_count}): {e}")
                if attempt < self.retry_count - 1:
                    time.sleep(1)
...
class VideoFileRenamer:
    ...
    def __init__(self, api_key: str):
        ...
        self.brave_client = BraveSearchClient(api_key, self.cache)
        self.searxng_client = None
        if SEARXNG_URL:  # Инициализируем SearxNG клиент только если есть URL
            self.searxng_client = SearxNGClient(SEARXNG_URL, self.cache)
    ...
    def _combine_search_results(self, query: str) -> List[Dict]:
        """Объединяет результаты поиска из разных источников"""
        results = []
        
        # Сначала пробуем Brave
        brave_results = self.brave_client.search(query)
        if brave_results:
            results.extend(brave_results)
            self.stats['brave_results'] += 1
            logger.info(f"Got {len(brave_results)} results from Brave")

        # Затем SearxNG если он доступен и нужны дополнительные результаты
        if self.searxng_client and (not results or len(results) < 10):
            searxng_results = self.searxng_client.search(query)
            if searxng_results:
                results.extend(searxng_results)
                self.stats['searxng_results'] += 1
                logger.info(f"Got {len(searxng_results)} results from SearxNG")
        ...
        try:
            ...
            for filename in os.listdir(directory):
                ...
                try:
                    ...
                    search_results = self._combine_search_results(base_name)
...
def setup_environment() -> bool:
    ...
    try:
        ...
        # Проверяем доступность SearxNG если URL задан
        if SEARXNG_URL:
            try:
                response = requests.get(SEARXNG_URL, timeout=5)
                response.raise_for_status()
                logger.info("SearxNG server is available")
            except Exception as e:
                logger.warning(f"SearxNG server is not available: {e}. Will use only Brave Search.")
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--searxng-url',
        type=str,
        help='SearxNG server URL (can also be set via SEARXNG_URL environment variable)',
        required=False
    )
...
def main() -> int:
    ...
    try:
        ...
        # Устанавливаем API ключи и URL
        global API_KEY, SEARXNG_URL
        ...
        if SEARXNG_URL:
            logger.info(f"Using SearxNG server at {SEARXNG_URL}")
        else:
            logger.info("Using only Brave Search (no SearxNG URL provided)")
        ...
        print(f"Results from Brave: {renamer.stats['brave_results']}")
        if SEARXNG_URL:
            print(f"Results from SearxNG: {renamer.stats['searxng_results']}")
