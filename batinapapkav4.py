...
SEARXNG_URL = "http://bigtweak.kozow.com:7777"  # Ваш SearxNG сервер
USE_SEARXNG = True  # Включаем использование SearxNG
...
class VideoFileRenamer:
    ...
    def _combine_search_results(self, query: str) -> List[Dict]:
        ...
        # Затем SearxNG если включен и нужны дополнительные результаты
...
def setup_environment() -> bool:
    ...
    try:
        ...
        # Проверяем доступность SearxNG если он включен
        if USE_SEARXNG:
            try:
                ...
                logger.warning(f"SearxNG server is not available: {e}. Continuing without it.")
                global USE_SEARXNG
                USE_SEARXNG = False
...
def main() -> int:
    ...
    try:
        ...
        # Отключаем SearxNG если указан флаг
        if args.no_searxng:
            global USE_SEARXNG
            USE_SEARXNG = False
            logger.info("SearxNG search disabled")
